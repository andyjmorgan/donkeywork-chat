...
import { AgentModel, AgentNode as AgentNodeType, AgentEdge } from '../../services/api/agentService';
...
export const AgentBuilder = ({ 
  ...
  useEffect(() => {
      ...
      if (agentId) {
        try {
          ...
          if (agent.nodes && agent.nodes.length > 0) {
            const flowNodes = agent.nodes.map(node => ({
              id: node.id,
              type: node.type || '',
            ...
            // Set nodes state with loaded nodes
            setNodes(flowNodes);
            
            // Set edges state with loaded edges
            if (agent.edges && agent.edges.length > 0) {
              setEdges(agent.edges);
            }
            
            // Find the highest node ID to continue incrementing from there
            const maxNodeId = Math.max(...agent.nodes.map(n => parseInt(n.id)));
          ...
          toast.current?.show({
            ...
            detail: `Successfully loaded agent: ${agent.name}`
          });
        } catch (error) {
          ...
          toast.current?.show({
            severity: 'error',
            summary: 'Error',
            detail: 'Failed to load agent. Please try again.'
          });
        }
      }
    };
    ...
    loadAgent();
  }, [agentId, setNodes, setEdges, setNodeId]);
  ...
  const saveAgent = useCallback(async () => {
    ...
    if (nameError) {
      ...
      setAgentConfigDialogVisible(true);
      return;
    }
    ...
    if (validationResult.valid) {
      ...
      const cleanedEdges: AgentEdge[] = edges.map(edge => ({
        id: edge.id,
        source: edge.source,
        target: edge.target,
      ...
      try {
        setIsSaving(true);
        let savedAgent: AgentModel;
        
        if (agentId) {
          // Update existing agent
          savedAgent = await agentService.updateAgent(agentId as string, agentData);
          toast.current?.show({
            severity: 'success',
            summary: 'Success',
          ...
          // Update agentId state with the new ID
        ...
        toast.current?.show({
          severity: 'error',
          summary: 'Error',
          detail: 'Failed to save agent. Please try again.'
        });
      } finally {
        setIsSaving(false);
      }
  ...
  }, [nodes, edges, onSave, agentName, agentDescription, agentTags, validateAgentName, agentId]);
      ...
      <div className="agent-card-header">
        ...
        <div className="agent-toolbar">
          ...
          <Button 
            ...
            onClick={() => {
              // Handle exit - you can add confirmation or other logic here if needed
              window.history.back();
            }} 
          />
          <Button 
