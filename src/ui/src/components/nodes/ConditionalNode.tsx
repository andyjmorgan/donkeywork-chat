    ...
    <>
      ...
      >
        ...
        <div className="node-content" onClick={handleOpenEditor}>
          <div className="condition-list">
            {conditions.length > 0 ? (
              <>
                {conditions.slice(0, 2).map((cond, index) => (
                ...
                {conditions.length > 2 && (
                  <div className="condition-item more-conditions">
                    <div className="condition-expression">
                      <span className="condition-index">+{conditions.length - 2}</span>
                      <code>more conditions...</code>
                    </div>
                  </div>
                )}
              </>
      ...
      >
        <div className="field" style={{ marginTop: '1rem' }}>
          ...
          {!validationResult.valid && (
            <div className="validation-error-list" style={{ marginBottom: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {validationResult.errors.map((error, index) => (
                <div 
                  key={index}
                  className={`custom-${error.severity === 'error' ? 'error' : 'warning'}-message`}
                  style={{
                    padding: '0.75rem',
                    backgroundColor: error.severity === 'error' 
                      ? 'var(--red-50, #fef2f2)' 
                      : 'var(--yellow-50, #fff8e6)',
                    borderRadius: '4px',
                    borderLeft: `4px solid ${error.severity === 'error' 
                      ? 'var(--red-500, #ef4444)' 
          ...
          <div className="conditions-section">
            ...
            <Message
              ...
              className="default-path-message condition-info-message"
            >
              <h5 style={{ margin: '0 0 0.5rem 0', color: 'var(--primary-color, #4F46E5)' }}>Default Path</h5>
          ...
          <div className="help-section">
            ...
            {(() => {
              ...
              if (connectedInputs.length === 0) {
                return (
                  <div className="custom-warning-message" style={{
                    margin: '0.5rem 0',
                    padding: '0.75rem',
                    backgroundColor: 'var(--yellow-50, #fff8e6)',
                    borderRadius: '4px',
                    borderLeft: '4px solid var(--yellow-500, #f59e0b)',
                    color: 'var(--yellow-800, #92400e)'
                  }}>
                    No connected input nodes found. Connect nodes to this conditional block to use their values.
                  </div>
                );
              }
