...
import { TemplateNodeData, NodeTypeEnum, ValidationResult } from '../../types/nodes';
...
const TemplateNode = ({ id, data, selected }: NodeProps) => {
  ...
  const getConnectedInputNodes = useCallback(() => {
    ...
    // Find all input nodes connected to this template node
    return getEdges()
  ...
  // Validate this node - for now, a simple validation
  const validate = useCallback((): ValidationResult => {
    // Template nodes are always valid for now - we could add more validations later
  ...
  return (
    <>
      <BaseNode data={{...data, nodeType: NodeTypeEnum.TEMPLATE}} className={!validationResult.valid ? 'invalid-node' : ''}>
      ...
      >
        <div className="field" style={{ marginTop: '1rem' }}>
          ...
          {!validationResult.valid && (
            <div className="validation-error-list" style={{ marginBottom: '0.75rem' }}>
          ...
          <small style={{ display: 'block', marginBottom: '0.75rem', color: '#6c757d' }}>
          ...
          <div className="help-section">
              ...
              if (connectedInputs.length === 0) {
                return (
                  <div style={{
                    ...
                    No connected input nodes found. Connect nodes to this template to use their values.</p>
                  </div>
                );
              }
            ...
            <p><strong>Note:</strong> You can only reference nodes that are directly connected as inputs to this template node.</p>
